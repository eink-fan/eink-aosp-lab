...
int main() {
  neo2::eink::SnapshotPool empty_pool({});
  assert(empty_pool.Capacity() == 0);
  assert(!empty_pool.TryAcquire().has_value());
  assert(empty_pool.DropCount() == 1);

  neo2::eink::SnapshotPool null_pool({std::shared_ptr<neo2::eink::ComposeBuffer>{}});
  ...
  neo2::eink::SnapshotPool pool({first_buffer, second_buffer});
