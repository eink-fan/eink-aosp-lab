...
using neo2::eink::GrayscaleDelta;
using neo2::eink::MicroDeltaDecision;
using neo2::eink::MicroDeltaPolicy;
...
int main() {
  ...
  Require(!policy.Reconfigure(neo2::eink::MicroDeltaConfig::Defaults()),
          "equivalent configuration was reported as a change");
  auto invalid_config = neo2::eink::MicroDeltaConfig::Defaults();
  invalid_config.changed_ppm = 99;
  Require(!policy.Reconfigure(invalid_config), "invalid micro configuration was accepted");
  auto configured = neo2::eink::MicroDeltaConfig::Defaults();
  ...
  Require(policy.HasDeferredCandidate() && policy.Deadline() == start + 300ms,
          "settle deadline did not use the B001 default");
  ...
  neo2::eink::MicroDeltaConfig top_edge = neo2::eink::MicroDeltaConfig::Defaults();
  top_edge.chrome_edge = neo2::eink::MicroChromeEdge::kTop;
