...
using neo2::eink::FullPoolCaptureDecision;
using neo2::eink::FullPoolResampleConfig;
using neo2::eink::FullPoolObservationDecision;
using neo2::eink::FullPoolPipelineState;
using neo2::eink::FullPoolReleaseDecision;
using neo2::eink::FullPoolResamplePolicy;
...
int main() {
  ...
  Require(policy.ObserveFullPool(true, {.conversion_in_flight = false,
          ...
          "conversion-free state escaped the narrow B002 predicate");
  Require(policy.ObserveFullPool(true, {.conversion_in_flight = true,
                                         .pending_capture = false,
                                         .adapter_retained = false},
                                  start) == FullPoolObservationDecision::kIgnored,
          "pending-free state escaped the narrow B002 predicate");
  ...
  Require(policy.ObserveFullPool(true, adapter_retained, start + 3s) ==
          ...
          "adapter-retained state escaped the narrow B002 predicate");
