...
using neo2::eink::CaptureAdmissionDecision;
using neo2::eink::CaptureAdmissionReason;
using neo2::eink::CaptureAdmissionPolicy;
...
int main() {
  ...
  Require(!policy.Reconfigure(neo2::eink::CaptureAdmissionConfig::Defaults()),
  ...
  Require(policy.Evaluate(true, now + 90ms, now) ==
          ...
          "late replacement did not open inside the B001 window");
