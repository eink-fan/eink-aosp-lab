...
// frames. An Android backend associates a destination GraphicBuffer and its
// draw-completion fence with the lease externally; this core deliberately has
// no Android fence or allocator dependency.
class SnapshotPool {
 public:
  // Empty or null-buffer input creates a zero-capacity fail-closed pool. This
  // keeps the portable core usable in Android targets built without C++
