...
public final class SleepImageCatalogService extends Service {
    public static final int PRESENTATION_WIDTH = 1072;
    public static final int PRESENTATION_HEIGHT = 1448;
    ...
    private static final int PROVIDER_PROTOCOL_VERSION = 4;
    ...
    final class UiBinder extends Binder {
        ...
        void commit(long expectedGeneration, byte[] gray, byte[] alpha, UiCallback callback) {
            ...
            mWorker.post(() -> {
                ...
                synchronized (mStoreLock) {
                    mutation = mStore.add(expectedGeneration, ownedGray, ownedAlpha);
                }
    ...
    private SleepImageStore.Snapshot ensureCatalogOnWorker() {
        synchronized (mStoreLock) {
            try {
                if (mFallback == null) {
                    mFallback = SleepImageTransforms.builtInFallback(WIDTH, HEIGHT);
                }
    ...
    private CatalogView catalogView(SleepImageStore.Snapshot snapshot) {
        ...
        for (SleepImageStore.Summary summary : mStore.summaries(snapshot, 134, 181)) {
    ...
    private void publishResult(long requestGeneration, int mode, IBinder callback) {
        ...
        if (publication.isEmpty()) {
            sendResult(callback, requestGeneration, snapshot.generation, RESULT_EMPTY,
                    REASON_NO_VALID_CANDIDATE, publication);
            return;
        }
        if (!sendResult(callback, requestGeneration, snapshot.generation, RESULT_OK, REASON_NONE,
            ...
            sendResult(callback, requestGeneration, snapshot.generation, RESULT_UNAVAILABLE,
                    REASON_SHARED_MEMORY, new ArrayList<>());
        }
    }
        ...
        try {
            ...
            if (result == RESULT_OK) {
                for (SleepImageStore.PublicationEntry candidate : candidates) {
                    SharedMemory grayEntry = SharedMemory.create("neo2-sleep-image-gray", BYTES);
                    SharedMemory alphaEntry = SharedMemory.create("neo2-sleep-image-alpha", BYTES);
                    try {
                        writeSharedMemory(grayEntry, candidate.entry.gray);
