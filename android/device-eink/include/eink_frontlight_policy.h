...
struct DirectFrontLightResolution {
  std::size_t cold_step = 0;
  std::size_t warm_step = 0;
  std::uint16_t cold_drive_code = 0;
  std::uint16_t warm_drive_code = 0;
...
// Maps one overall brightness and hue request directly through a device's
// observed 31-step controller table. The cold and warm step indices partition
// the requested intensity, so the routine never requests both channels at
// their independent maximum. This is for devices whose stock firmware exposes
// controller codes but no trustworthy electrical power curve.
[[nodiscard]] std::optional<DirectFrontLightResolution> ResolveDirectFrontLightPercent(
        const std::array<std::uint16_t, kFrontLightSteps>& brightness_codes,
        int brightness_percent, int warmth_percent);

[[nodiscard]] bool IsValidFrontLightBrightnessCodes(
        const std::array<std::uint16_t, kFrontLightSteps>& brightness_codes);
