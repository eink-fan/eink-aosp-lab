...
bool PresentationAdapter::Enqueue(Frame frame) {
  {
    ...
    if (impl_->pending.has_value()) {
      ...
      // Follow the newest awake target if capture coalesces before submission.
      // A sleep frame cancels the diagnostic rather than flashing its cover.
      if (frame.sleep_image_epoch == 0 && frame.diagnostic_request == 0) {
        frame.diagnostic_refresh = impl_->pending->diagnostic_refresh;
        frame.diagnostic_request = impl_->pending->diagnostic_request;
      }
    }
...
bool PresentationAdapter::CancelPendingDiagnosticRefresh() {
  std::lock_guard lock(impl_->mutex);
  if (!impl_->pending || impl_->pending->diagnostic_request == 0) return false;
  impl_->pending->diagnostic_request = 0;
  impl_->pending->diagnostic_refresh = DiagnosticRefresh::kNone;
  return true;
}
