...
bool SyncControllerBackend::InitializeForObservedPanel() {
  ...
  ALOGI("M4 vendor Sync init begin: geometry=%ux%u", kDeviceProfile.panel_width,
        kDeviceProfile.panel_height);
  sync->init(kDeviceProfile.panel_width, kDeviceProfile.panel_height);
  impl_->sync = sync;
...
bool SyncControllerBackend::Submit(const Frame& frame) {
  ...
  hwc_epdc_llist record{
          .rect = ::android::Rect(0, 0, static_cast<int>(kDeviceProfile.panel_width),
