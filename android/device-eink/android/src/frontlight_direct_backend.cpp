...
constexpr std::string_view kCalibrationPath = BuildEinkDeviceProfile().frontlight_calibration_path;
struct DebugRequest {
  int cold_code = 0;
  int warm_code = 0;
...
bool ParsePayload(const std::string& contents, Payload* payload) {
  bool calibrated_schema_seen = false;
  bool direct_schema_seen = false;
  ...
  for (const auto& raw_line : ::android::base::Split(contents, "\n")) {
    ...
    if (key == "schema") {
      calibrated_schema_seen = value == "neo2-frontlight-v1";
      direct_schema_seen = value == "neo2-frontlight-direct-v1";
  ...
  const bool common_valid = !payload->calibration_id.empty() && brightness_seen &&
          !payload->cold_primary_path.empty() && !payload->warm_primary_path.empty();
  if (direct_schema_seen && !calibrated_schema_seen && common_valid && !cold_seen && !warm_seen &&
      !drive_codes_seen && !drive_power_seen && payload->max_power_multiple == 0.0) {
    payload->direct_codes = true;
    return IsValidFrontLightBrightnessCodes(payload->calibration.brightness_codes);
  }
  return calibrated_schema_seen && !direct_schema_seen && common_valid && cold_seen && warm_seen &&
          drive_codes_seen && drive_power_seen && std::isfinite(payload->max_power_multiple) &&
...
bool WriteCode(const std::string& path, std::uint16_t code) {
  ...
  // Aura's LM3630 group attributes consume only the first digit when a
  // multi-digit write is not newline-terminated. A normal sysfs text record
  // is valid on the retained monochrome controllers as well.
...
bool FrontLightDirectBackend::Poll() {
  ...
  if (!impl_->payload) {
    std::string contents;
  if (!::android::base::ReadFileToString(std::string(kCalibrationPath), &contents)) {
    ...
    if ((!payload.direct_codes && !IsValidFrontLightCalibration(payload.calibration)) ||
        (payload.direct_codes &&
         !IsValidFrontLightBrightnessCodes(payload.calibration.brightness_codes))) {
  ...
  std::optional<FrontLightResolution> resolution;
  if (payload.direct_codes) {
    const auto direct = ResolveDirectFrontLightPercent(
            payload.calibration.brightness_codes, 0, 0);
    if (direct) {
      resolution = FrontLightResolution{
              .cold_step = direct->cold_step,
              .warm_step = direct->warm_step,
              .cold_drive_step = direct->cold_step,
              .warm_drive_step = direct->warm_step,
              .normalized_cold_code = direct->cold_drive_code,
              .normalized_warm_code = direct->warm_drive_code,
              .cold_drive_code = direct->cold_drive_code,
              .warm_drive_code = direct->warm_drive_code,
      };
    }
  } else {
    resolution = ResolveFrontLight(
...
neo2::eink::FrontLightControlOutcome FrontLightDirectBackend::ApplyManualRequest(
  ...
  std::optional<FrontLightResolution> resolution;
  if (payload.direct_codes) {
    const auto direct = ResolveDirectFrontLightPercent(payload.calibration.brightness_codes,
                                                       request.brightness_percent,
                                                       request.warmth_percent);
    if (direct) {
      resolution = FrontLightResolution{
              .cold_step = direct->cold_step,
              .warm_step = direct->warm_step,
    ...
    resolution = ResolveFrontLightPercent(payload.calibration, request.brightness_percent,
