...
auto DrawLayersCompat(
        RenderEngine& render_engine,
        const ::android::renderengine::DisplaySettings& display,
        const std::vector<::android::renderengine::LayerSettings>& layers,
        const std::shared_ptr<::android::renderengine::ExternalTexture>& destination) {
  if constexpr (requires {
                  render_engine.drawLayers(display, layers, destination, false,
                                           ::android::base::unique_fd());
                }) {
    return render_engine.drawLayers(display, layers, destination, false,
                                    ::android::base::unique_fd());
  } else {
    return render_engine.drawLayers(display, layers, destination,
                                    ::android::base::unique_fd());
  }
}
...
std::optional<GpuSnapshotPool::Snapshot> GpuSnapshotPool::TryCopy(
  ...
  const auto fence_result = DrawLayersCompat(render_engine_, display, {layer}, destination).get();
  if (!fence_result.ok()) {
    std::lock_guard lock(mutex_);
    Slot& slot = slots_[slot_index];
    if (slot.state == SlotState::kCopying && slot.generation == generation) {
