...
void RecordDeltaDiagnostics(AndroidEinkPipeline::Diagnostics* diagnostics,
  ...
  if (delta_unavailable) {
    ...
    diagnostics->latest_delta_changed_pixels = 0;
    diagnostics->latest_delta_total_pixels = 0;
    diagnostics->latest_delta_coverage_ppm = 0;
    diagnostics->latest_delta_left = diagnostics->latest_delta_top = 0;
    diagnostics->latest_delta_right = diagnostics->latest_delta_bottom = 0;
    return;
  }
  if (!delta) return;
...
class AndroidEinkPipeline::Impl {
 public:
  Impl()
      : profile(BuildEinkDeviceProfile()),
        sleep_image_background(static_cast<int>(profile.panel_width),
                               static_cast<int>(profile.panel_height)) {}

  const EinkDeviceProfile& profile;
  std::mutex mutex;
  ...
  RgbFrameTracker rgb_tracker;
  GrayscaleFrameTracker grayscale_tracker;
  DiagnosticRefresh refresh_mode = DiagnosticRefresh::kNone;
  std::uint64_t refresh_id = 0;
  ...
  SleepImageBackground sleep_image_background;
...
std::uint64_t AndroidEinkPipeline::RequestDiagnosticRefresh(DiagnosticRefresh mode) {
  std::lock_guard lock(impl_->mutex);
  if (!impl_->profile.has_color_cfa || !impl_->started || impl_->stopping ||
      !impl_->enabled || impl_->sleep_image_latch.active_epoch() != 0 ||
      impl_->refresh_busy || mode == DiagnosticRefresh::kNone) return 0;
  impl_->refresh_busy = true;
  impl_->refresh_id = ++impl_->diagnostics.refresh_requested;
  impl_->refresh_mode = mode;
  return impl_->refresh_id;
}

void AndroidEinkPipeline::CancelDiagnosticRefresh(std::uint64_t id) {
  std::lock_guard lock(impl_->mutex);
  if (id != 0 && impl_->refresh_id == id) {
    impl_->diagnostics.refresh_cancelled = id;
    impl_->refresh_id = 0;
    impl_->refresh_busy = false;
    impl_->refresh_mode = DiagnosticRefresh::kNone;
  }
}
...
void AndroidEinkPipeline::SetEnabled(bool enabled) {
  ...
  if (impl_->started && !impl_->stopping) {
    ...
    if (!enabled) {
      const bool cancelled = adapter_.CancelPendingDiagnosticRefresh();
      if (cancelled || impl_->refresh_id != 0)
        impl_->diagnostics.refresh_cancelled = impl_->diagnostics.refresh_requested;
      impl_->refresh_busy = false;
      impl_->refresh_id = 0;
      impl_->refresh_mode = DiagnosticRefresh::kNone;
    }
...
EinkSleepImageCatalog::PublishResult AndroidEinkPipeline::PublishSleepImageCatalog(
  ...
  if (width != static_cast<int>(impl_->profile.panel_width) ||
      height != static_cast<int>(impl_->profile.panel_height)) {
...
SleepImagePresentationResult AndroidEinkPipeline::PresentSleepImage(std::uint64_t epoch) {
  ...
  const std::size_t panel_bytes = EinkSleepImageCatalog::PanelBytesFor(impl_->profile);
  if (!candidate.buffer || candidate.width != static_cast<int>(impl_->profile.panel_width) ||
      candidate.height != static_cast<int>(impl_->profile.panel_height) ||
      candidate.buffer->ByteCount() != panel_bytes || !candidate.alpha ||
      candidate.alpha->size() != panel_bytes) {
    return reject(SleepImagePresentationResult::kRejectedCatalog);
  }

  std::shared_ptr<OwnedGrayscaleBuffer> presentation;
  std::shared_ptr<const std::vector<std::uint8_t>> color_presentation;
  using ByteSpan = std::span<const std::uint8_t>;
  const auto foreground_bytes =
          static_cast<decltype(ByteSpan{}.size())>(candidate.buffer->ByteCount());
  const ByteSpan foreground(candidate.buffer->Data(), foreground_bytes);
  const std::span alpha(*candidate.alpha);
  const ByteSpan rgba = candidate.rgba ? ByteSpan(*candidate.rgba) : ByteSpan{};
  if (impl_->sleep_image_mode == SleepImagePresentationMode::kOverlay) {
    SleepImagePreparedOverlay prepared =
            impl_->sleep_image_background.PrepareOverlay(foreground, alpha, rgba);
    ...
    if (!prepared.rgba.empty()) {
      color_presentation = std::make_shared<const std::vector<std::uint8_t>>(
              std::move(prepared.rgba));
    }
  } else {
    const std::vector<std::uint8_t> white(panel_bytes, 0xf0);
    SleepImageOverlayOutput prepared = ComposeSleepImageOverlay(foreground, alpha, white, rgba);
  ...
  Frame frame{
          ...
          .sleep_image_rgba = std::move(color_presentation),
          .dirty_rect = {.left = 0,
                         .top = 0,
...
void AndroidEinkPipeline::OnEngineSubmission(
  ...
  if (frame.diagnostic_request != 0) {
    if (accepted) impl_->diagnostics.refresh_submitted = frame.diagnostic_request;
    if (frame.diagnostic_request == impl_->diagnostics.refresh_requested) impl_->refresh_busy = false;
    if (!accepted) impl_->diagnostics.refresh_failed = frame.diagnostic_request;
  }
  if (frame.engine_mode == engine_mode_ && frame.diagnostic_request == 0) {
    if (accepted) {
  ...
  while (true) {
    ...
    std::uint64_t source_generation = 0;
    {
      ...
      if (impl_->reset_source_state) {
        ...
        impl_->rgb_tracker.Reset();
        impl_->grayscale_tracker.Reset();
    ...
    std::uint64_t rgb_signature = 0;
    const auto snapshot_grayscale = converter_.Convert(*composition, rotation_,
            impl_->profile.has_color_cfa ? &rgb_signature : nullptr,
            impl_->profile.has_color_cfa ? &impl_->rgb_tracker : nullptr);
    ...
    if (!snapshot_grayscale ||
      ...
      bool first_failure = false;
      {
        ...
        if (impl_->refresh_id != 0) {
          impl_->diagnostics.refresh_failed = impl_->refresh_id;
          impl_->refresh_id = 0;
          impl_->refresh_mode = DiagnosticRefresh::kNone;
          impl_->refresh_busy = false;
        }
    ...
    // Demand uses exact RGB (color) or final panel-byte (mono) comparison.
    // Hashing and delta scans are diagnostic-only on both paths.
    const auto grayscale_signature = impl_->profile.has_color_cfa ? 0 :
            impl_->grayscale_tracker.Update(*grayscale);
    const bool gray_diagnostics =
            ::android::base::GetBoolProperty("debug.neo2.eink.gray_diagnostics", false);
    const std::uint64_t grayscale_hash = gray_diagnostics ? FingerprintGrayscale(*grayscale) : 0;
    const std::uint64_t capture_to_convert_ns =
            ToNanoseconds(conversion_finish - composition->captured_at);
    std::optional<GrayscaleDelta> delta;
    bool delta_unavailable = !gray_diagnostics;
    if (gray_diagnostics && impl_->has_previous_grayscale) {
    ...
    const auto demand = impl_->demand_gate.Evaluate({
            .hash = impl_->profile.has_color_cfa ? rgb_signature : grayscale_signature,
    ...
    if (sleep_decision == SleepImageDecision::kSuppress) {
      ...
      impl_->diagnostics.conversion_duration_max_ns =
              std::max(impl_->diagnostics.conversion_duration_max_ns, convert_ns);
      impl_->conversion_in_flight = false;
      impl_->diagnostics.grayscale_diagnostics_enabled = gray_diagnostics;
    ...
    {
      std::lock_guard lock(impl_->mutex);
      if (impl_->enabled && impl_->sleep_image_latch.active_epoch() == 0) {
        refresh_mode = impl_->refresh_mode;
        refresh_id = impl_->refresh_id;
      }
    }
    ...
    // Monochrome output releases an owned snapshot after conversion because
    // its lower engine consumes only the owned gray bytes. Aura C instead
    // moves that snapshot's lease into AndroidComposeBuffer so the exact RGBA
    // source and its derived gray plane remain paired until Submit returns.
    const bool is_owned_snapshot = static_cast<bool>(composition->ownership_lease);
    std::shared_ptr<ComposeBuffer> engine_compose;
    if constexpr (BuildEinkDeviceProfile().has_color_cfa) {
      engine_compose = std::make_shared<AndroidComposeBuffer>(
              std::move(composition->compose_buffer), ::android::Fence::NO_FENCE,
              std::move(composition->ownership_lease));
    } else if (!is_owned_snapshot) {
      engine_compose = std::make_shared<AndroidComposeBuffer>(
    ...
    Frame frame{
            ...
            .sleep_image_epoch = sleep_decision == SleepImageDecision::kReplace
                    ...
                    : 0,
            .captured_at = composition->captured_at,
            .compose_buffer = std::move(engine_compose),
    ...
    frame.diagnostic_refresh = refresh_mode;
    frame.diagnostic_request = refresh_id;
    composition.reset();
    ...
    {
      std::lock_guard lock(impl_->mutex);
      // Sleep/disable may cancel the request while the owned frame is being
      // assembled. Serialize the final check and enqueue with cancellation.
      if (refresh_id != 0 && (impl_->refresh_id != refresh_id || !impl_->enabled ||
                             impl_->sleep_image_latch.active_epoch() != 0)) {
        frame.diagnostic_request = 0;
      ...
      if (refresh_id == 0) ++impl_->diagnostics.normal_differential_selected;
      enqueued = adapter_.Enqueue(std::move(frame));
      if (!enqueued && refresh_id != 0) {
        impl_->refresh_busy = false;
        impl_->diagnostics.refresh_failed = refresh_id;
      }
    }
