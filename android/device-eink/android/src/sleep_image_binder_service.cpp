...
// Aura v3 transports RGBA + alpha; monochrome v2 remains gray + alpha.
constexpr std::int32_t kProtocolVersion = kDeviceProfile.has_color_cfa ? 3 : 2;
constexpr std::size_t kPanelBytes = EinkSleepImageCatalog::PanelBytesFor(kDeviceProfile);
...
bool ReadPlane(const ::android::Parcel& data, std::vector<std::uint8_t>* output,
               std::size_t expected_bytes = kPanelBytes) {
  ...
  if (shared_memory_size != expected_bytes) {
    return false;
  }
  ...
  void* mapped = mmap(nullptr, expected_bytes, PROT_READ, MAP_SHARED, fd, 0);
  if (mapped == MAP_FAILED) return false;
  const auto* bytes = static_cast<const std::uint8_t*>(mapped);
  output->assign(bytes, bytes + expected_bytes);
  munmap(mapped, expected_bytes);
  return true;
}
...
bool ReadEntry(const ::android::Parcel& data, EinkSleepImageCatalog::Entry* entry) {
  ...
  if (entry->width != static_cast<int>(kDeviceProfile.panel_width) ||
      entry->height != static_cast<int>(kDeviceProfile.panel_height)) return false;
  if constexpr (kDeviceProfile.has_color_cfa) {
    if (!ReadPlane(data, &entry->rgba, kPanelBytes * 4) ||
        !ReadPlane(data, &entry->alpha)) return false;
    entry->panel_gray.resize(kPanelBytes);
    for (std::size_t index = 0; index < kPanelBytes; ++index) {
      const auto* rgb = entry->rgba.data() + 4 * index;
      if (rgb[3] != entry->alpha[index]) return false;
      entry->panel_gray[index] = static_cast<std::uint8_t>(
              ((77U * rgb[0] + 150U * rgb[1] + 29U * rgb[2]) >> 8) & 0xf0U);
    }
    return true;
  }
  return ReadPlane(data, &entry->panel_gray) && ReadPlane(data, &entry->alpha);
}
 ...
 public:
  ...
  ::android::status_t onTransact(std::uint32_t code, const ::android::Parcel& data,
    ...
    const auto result = g_session->PublishSleepImageCatalog(
            epoch, static_cast<int>(kDeviceProfile.panel_width),
