...
#if defined(NEO2_EINK_M4_LOWER_ENGINE_OUTPUT)
  if constexpr (!BuildEinkDeviceProfile().has_color_cfa) return -1;
  if (enabled == 0 || enabled == 1) SetColorQualityEnabled(enabled == 1);
  return GetColorQualityEnabled() ? 1 : 0;
#else
...
#if defined(NEO2_EINK_M4_LOWER_ENGINE_OUTPUT)
  if constexpr (!BuildEinkDeviceProfile().has_color_cfa) return {};
  if (requested != -1 && !IsGrayPreference(requested)) return {};
  std::lock_guard lock(g_fresh_snapshot_session_mutex);
  const int previous = static_cast<int>(GetGrayPreference());
  if (requested == -1 || requested == previous) return {previous, false};
  SetGrayPreference(static_cast<GrayPreference>(requested));
  const auto receipt = g_fresh_snapshot_session
          ? g_fresh_snapshot_session->RequestDiagnosticRefresh("differential") : "";
  // Mode remains selected if asleep/busy; the next eligible frame uses it.
  // Report queue admission honestly, never physical completion.
  return {requested, receipt.starts_with("refresh queued id=")};
#else
...
#if defined(NEO2_EINK_M4_LOWER_ENGINE_OUTPUT)
  if constexpr (!BuildEinkDeviceProfile().has_color_cfa) return {};
  if (requested != -1 && (requested < 0 || requested > 1)) return {};
  std::lock_guard lock(g_fresh_snapshot_session_mutex);
  const int previous = static_cast<int>(GetVividColorEnabled());
  if (requested == -1 || requested == previous) return {previous, false};
  SetVividColorEnabled(requested == 1);
  const auto receipt = g_fresh_snapshot_session
          ? g_fresh_snapshot_session->RequestDiagnosticRefresh("full") : "";
  // Mode remains selected if asleep/busy; the next eligible frame uses it.
  // Report queue admission honestly, never physical completion.
  return {requested, receipt.starts_with("refresh queued id=")};
#else
...
std::string RequestNeo2EinkColorCleanup() {
  std::lock_guard lock(g_fresh_snapshot_session_mutex);
  return g_fresh_snapshot_session ? g_fresh_snapshot_session->RequestDiagnosticRefresh("color-clean")
                                  : "refresh unavailable\n";
}

std::string RequestNeo2EinkFullRefresh() {
  std::lock_guard lock(g_fresh_snapshot_session_mutex);
  return g_fresh_snapshot_session ? g_fresh_snapshot_session->RequestDiagnosticRefresh("white")
                                  : "refresh unavailable\n";
}

std::string RequestNeo2EinkFullWaveformRefresh() {
  std::lock_guard lock(g_fresh_snapshot_session_mutex);
  return g_fresh_snapshot_session ? g_fresh_snapshot_session->RequestDiagnosticRefresh("full")
                                  : "refresh unavailable\n";
}

std::string RequestNeo2EinkDiagnosticRefresh(const std::string& mode) {
  if (!::android::base::GetBoolProperty("ro.debuggable", false)) return "refresh disabled\n";
  if (mode == "quality-on" || mode == "quality-off") {
    const int result = ConfigureNeo2EinkColorQuality(mode == "quality-on" ? 1 : 0);
    return result == 1 ? "quality enabled\n" : result == 0 ? "quality disabled\n" : "quality unavailable\n";
  }
  if (mode == "quality-status")
    return ConfigureNeo2EinkColorQuality(-1) == 1 ? "quality enabled\n" : "quality disabled\n";
  std::lock_guard lock(g_fresh_snapshot_session_mutex);
  return g_fresh_snapshot_session ? g_fresh_snapshot_session->RequestDiagnosticRefresh(mode)
                                  : "refresh unavailable\n";
}
...
RenderSurfaceCaptureSession::RenderSurfaceCaptureSession(
  ...
  if (!PublishSleepImageBinderService()) {
    ALOGE("Unable to publish the Neo2 sleep-image endpoint");
  }
  if (!PublishManualFrontLightBinderService()) {
    ALOGE("Unable to publish the Neo2 front-light endpoint");
  }
}
...
EinkSleepImageCatalog::PublishResult RenderSurfaceCaptureSession::PublishSleepImageCatalog(
  ...
  if (width != static_cast<int>(BuildEinkDeviceProfile().panel_width) ||
      height != static_cast<int>(BuildEinkDeviceProfile().panel_height)) {
...
std::string RenderSurfaceCaptureSession::RequestDiagnosticRefresh(const std::string& mode) {
  if (!m3_ || !request_composite_) return "refresh unavailable\n";
  std::shared_ptr<AndroidEinkPipeline> pipeline;
  { std::lock_guard lock(m3_->sleep_image_mutex); pipeline = m3_->pipeline; }
  if (!pipeline) return "refresh unavailable\n";
  if (mode == "status") {
    const auto d = pipeline->diagnostics();
    return "refresh requested=" + std::to_string(d.refresh_requested) +
            " submitted=" + std::to_string(d.refresh_submitted) +
            " failed=" + std::to_string(d.refresh_failed) +
            " cancelled=" + std::to_string(d.refresh_cancelled) + " completion=unknown\n";
  }
  const auto selected = mode == "differential" ? DiagnosticRefresh::kDifferential :
          mode == "full" ? DiagnosticRefresh::kFull :
          mode == "white" ? DiagnosticRefresh::kWhiteThenTarget :
          mode == "color-clean" ? DiagnosticRefresh::kColorCleanup : DiagnosticRefresh::kNone;
  if (selected == DiagnosticRefresh::kNone) return "refresh modes: differential full white color-clean status quality-on quality-off quality-status\n";
  const auto id = pipeline->RequestDiagnosticRefresh(selected);
  if (id == 0) return "refresh rejected: unavailable, asleep, disabled, or pending\n";
  if (!request_composite_()) {
    pipeline->CancelDiagnosticRefresh(id);
    return "refresh rejected: composition request failed\n";
  }
  return "refresh queued id=" + std::to_string(id) + " mode=" + mode + "\n";
}
...
void RenderSurfaceCaptureSession::Capture(
  ...
  if (metadata->geometry_status != reported_geometry_status_) {
    ...
    if (reported_geometry_status_ == PanelGeometryStatus::kVerified) {
      ...
      ALOGE("metadata capture geometry rejected: got %ux%u, expected %ux%u; "
            "pixel work remains disabled",
            metadata->width, metadata->height, sink_.ExpectedWidth(), sink_.ExpectedHeight());
    }
  }
...
void RenderSurfaceCaptureSession::MaybeStartM3() {
  ...
  if (render_engine_ == nullptr || !sink_.HasRecognizedProfile() ||
      expected_width_ != sink_.ExpectedWidth() || expected_height_ != sink_.ExpectedHeight()) {
    ALOGE("M3 snapshot disabled: RenderSurface dimensions are %ux%u, expected recognized %ux%u",
          expected_width_, expected_height_, sink_.ExpectedWidth(), sink_.ExpectedHeight());
...
#else
          PresentationAdapter::SubmissionIntervalProvider{}
#endif
  );
...
#if defined(NEO2_EINK_M4_LOWER_ENGINE_OUTPUT)
  ALOGI("M4 lower-engine refresh controller armed: %zu owned RGBA destinations at %ux%u "
...
#else
  ALOGI("M3 capture-only snapshot enabled: %zu owned RGBA destinations at %ux%u",
        m3_->snapshot_pool->capacity(), expected_width_, expected_height_);
#endif
}
...
void RenderSurfaceCaptureSession::MaybeLogM3Diagnostics() {
  ...
  const int snapshot_size = std::snprintf(
          ...
          "gray_diagnostics=%d delta_bucket_counts=%llu/%llu/%llu/%llu/%llu delta_last_ppm=%u "
          "delta_last_box=%d,%d,%d,%d capture_to_submit_ns=%llu"
#if defined(NEO2_EINK_M4_LOWER_ENGINE_OUTPUT)
          " quality=%llu/%llu/%llu lower_rejected=%llu lower_idle=%llu/%llu/%llu/%llu completion_unknown=%llu "
          ...
          diagnostics.grayscale_diagnostics_enabled ? 1 : 0,
...
#if defined(NEO2_EINK_M4_LOWER_ENGINE_OUTPUT)
          , static_cast<unsigned long long>(lower_diagnostics.quality_sequences),
          static_cast<unsigned long long>(lower_diagnostics.quality_whitened_pixels),
          static_cast<unsigned long long>(lower_diagnostics.latest_quality_ns),
