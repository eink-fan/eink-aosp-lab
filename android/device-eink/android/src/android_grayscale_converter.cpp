...
std::shared_ptr<OwnedGrayscaleBuffer> AndroidGrayscaleConverter::Convert(
        ...
        std::uint64_t* rgb_signature, RgbFrameTracker* rgb_tracker) const {
  ...
  if (grayscale && rgb_signature) {
    *rgb_signature = rgb_tracker ? rgb_tracker->Update(source) : FingerprintRgb(source);
  }
