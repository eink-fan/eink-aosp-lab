...
class AndroidComposeBuffer final : public ComposeBuffer {
 public:
  ...
  [[nodiscard]] bool owns_snapshot_lease() const {
    return ownership_lease_ != nullptr;
  }
...
class AndroidEinkPipeline final : public CaptureSink {
 public:
  struct Diagnostics {
    std::uint64_t refresh_requested = 0;
    std::uint64_t refresh_submitted = 0;
    std::uint64_t refresh_failed = 0;
    std::uint64_t refresh_cancelled = 0;
    ...
    bool grayscale_diagnostics_enabled = false;
  ...
  [[nodiscard]] std::uint64_t RequestDiagnosticRefresh(DiagnosticRefresh mode);
  void CancelDiagnosticRefresh(std::uint64_t id);
