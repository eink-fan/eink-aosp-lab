...
class CaptureMetadataSink final : public CaptureSink {
 public:
  explicit CaptureMetadataSink(
          const EinkDeviceProfile& profile = BuildEinkDeviceProfile());
  CaptureMetadataSink(std::uint32_t expected_width, std::uint32_t expected_height);
  ...
  [[nodiscard]] std::uint32_t ExpectedWidth() const;
  [[nodiscard]] std::uint32_t ExpectedHeight() const;
  [[nodiscard]] bool HasRecognizedProfile() const;
  [[nodiscard]] std::optional<CaptureMetadata> LatestCapture() const;

 private:
  mutable std::mutex mutex_;
  EinkPanelGeometryGate geometry_gate_;
