...
bool GetColorQualityEnabled();
void SetColorQualityEnabled(bool enabled);
bool GetVividColorEnabled();
...
class LowerEngineDirectBackend final : public Engine {
 public:
  enum class State : std::uint8_t {
    ...
    kResolved,
  // iDisplayEngine_init and any profile-required color initialization
  // returned zero. The retained engine is process-lifetime state from here
  // onward: it must never be reinitialized, deinitialized, or dlclosed. The
  // M4-only executor defaults to a rate-limited continuous guest loop; an
  // operator may restore its finite startup-frame budget from this state.
    kInitialized,
  ...
  enum PreflightFailure : std::uint32_t {
    ...
    kColorRuntimeUnavailable = 1U << 8,
    kColorConfigurationUnavailable = 1U << 9,
  };

  struct Diagnostics {
    State state = State::kStopped;
    std::uint64_t quality_sequences = 0;
    std::uint64_t quality_whitened_pixels = 0;
    std::uint64_t latest_quality_ns = 0;
    ...
    bool waveform_copy_compatibility_applied = false;
    std::uint8_t waveform_copy_compatibility_result = 0;
    int initializer_result = 0;
    int color_initializer_result = 0;
    int color_aie_result = 0;
    int color_vpcoa_result = 0;
    int regal_support_result = 0;
    int regal_initializer_result = 0;
    int monochrome_initializer_result = 0;
    int update_result = 0;
  };
  ...
  // exact waveform/controller/pix-format prerequisites, calls
  // iDisplayEngine_init(), and performs the Aura-only color initialization
  // when selected. It is intentionally not reachable from the M1-M3
