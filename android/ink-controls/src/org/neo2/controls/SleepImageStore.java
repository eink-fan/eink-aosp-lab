...
final class SleepImageStore {
    ...
    private static final String LEGACY_RETIRED = "legacy-b027-retired.v1";
    ...
    static final class Entry {
        ...
        Entry(String id, boolean builtIn, boolean rotationEnabled, byte[] gray, byte[] alpha) {
            this(id, builtIn, rotationEnabled, gray, alpha, null);
        }
    ...
    private final int presentationToPanelRotation;
    private final String builtInId;
    private final FaultInjector faults;

    SleepImageStore(File root, int width, int height) {
        this(root, width, height, 270, BUILT_IN_ID, NO_FAULTS);
    }

    SleepImageStore(File root, int width, int height, FaultInjector faults) {
        this(root, width, height, 270, BUILT_IN_ID, faults);
    }

    SleepImageStore(File root, int width, int height, int presentationToPanelRotation) {
        this(root, width, height, presentationToPanelRotation, BUILT_IN_ID, NO_FAULTS);
    }

    SleepImageStore(File root, int width, int height, int presentationToPanelRotation,
            String builtInId) {
        this(root, width, height, presentationToPanelRotation, builtInId, NO_FAULTS);
    }

    SleepImageStore(File root, int width, int height, int presentationToPanelRotation,
            FaultInjector faults) {
        this(root, width, height, presentationToPanelRotation, BUILT_IN_ID, faults);
    }

    SleepImageStore(File root, int width, int height, int presentationToPanelRotation,
            String builtInId, FaultInjector faults) {
        this.root = root;
        this.width = width;
        this.height = height;
        this.bytes = Math.multiplyExact(width, height);
        if (presentationToPanelRotation != 0 && presentationToPanelRotation != 270) {
        ...
        if (!safeId(builtInId)) throw new IllegalArgumentException("unsafe built-in id");
    ...
    Snapshot openAndRepair(byte[] fallbackGray, byte[] fallbackAlpha) throws IOException {
        ...
        int builtInIndex = findBuiltIn(snapshot.entries);
        if (!snapshot.entries.isEmpty() && !snapshot.metadataConflict && builtInIndex >= 0) {
            return snapshot;
        }
        if (!snapshot.entries.isEmpty()) {
            ArrayList<Entry> repaired = new ArrayList<>(snapshot.entries);
            boolean rotationEnabled = true;
            int legacyBuiltIn = findLegacyBuiltIn(repaired);
            if (legacyBuiltIn >= 0) {
                rotationEnabled = repaired.get(legacyBuiltIn).rotationEnabled;
                repaired.remove(legacyBuiltIn);
            }
            if (findBuiltIn(repaired) < 0) {
                ...
                repaired.add(0, new Entry(builtInId, true, rotationEnabled,
        ...
        if (migrated.isEmpty()) {
            migrated.add(new Entry(builtInId, true,
                    fallbackGray.clone(), fallbackAlpha.clone()));
        } else if (migrated.size() < MAXIMUM_ENTRIES) {
            migrated.add(0, new Entry(builtInId, true,
                    fallbackGray.clone(), fallbackAlpha.clone()));
        }
    ...
    private Snapshot readManifest(File manifest) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(manifest))) {
            ...
            while ((line = reader.readLine()) != null && entries.size() < MAXIMUM_ENTRIES) {
                ...
                boolean authoritativeBuiltIn = fields[0].equals(builtInId);
                boolean legacyBuiltIn = isLegacyBuiltInId(fields[0]) && declaredBuiltIn;
                ...
                Entry entry = loadEntry(fields[0],
                        authoritativeBuiltIn || legacyBuiltIn, rotationEnabled);
    ...
    Mutation add(long expectedGeneration, byte[] gray, byte[] alpha) {
        return add(expectedGeneration, gray, alpha, null);
    }

    Mutation add(long expectedGeneration, byte[] gray, byte[] alpha, byte[] rgba) {
        try {
            ...
            if (rgba != null && rgba.length != Math.multiplyExact(bytes, 4)) {
                throw new IOException("invalid RGBA plane");
            }
            next.add(new Entry(id, false, true, gray.clone(), alpha.clone(),
                    rgba == null ? null : rgba.clone()));
            return new Mutation(MutationResult.COMMITTED,
                    commitEntries(current.generation, next));
        } catch (IOException | RuntimeException failure) {
            System.err.println("SleepImageStore import failed: " + failure);
    ...
    Mutation remove(long expectedGeneration, String id) {
        ...
        if (current.entries.get(selected).id.equals(builtInId)) {
    ...
    private void writeEntry(Entry entry) throws IOException {
        ...
        if (gray.isFile() && alpha.isFile()) {
            try {
                if (Arrays.equals(readExact(gray), entry.gray) &&
                        Arrays.equals(readExact(alpha), entry.alpha) &&
                        (entry.rgba == null || Arrays.equals(readColorPlane(entry.id), entry.rgba))) {
                    return;
                }
        ...
        try {
            ...
            if (entry.rgba != null) {
                if (entry.rgba.length != Math.multiplyExact(bytes, 4)) {
                    throw new IOException("invalid RGBA plane");
                }
                writeAtomic(rgbaFile(entry.id), entry.rgba, "rgba");
            }
        } catch (IOException failure) {
            gray.delete();
            throw failure;
    ...
    private byte[] manifestBytes(long generation, List<Entry> entries) {
        ...
        for (Entry entry : entries) {
            manifest.append(entry.id).append('\t')
                    .append(entry.id.equals(builtInId) ? '1' : '0').append('\t')
                    .append(entry.id.equals(builtInId) && !entry.rotationEnabled ? '0' : '1')
                    .append('\n');
        }
    ...
    private void pruneUnreferenced(List<Entry> retained) {
        ...
        for (Entry entry : retained) {
            ...
            names.add(rgbaFile(entry.id).getName());
        }
        File[] files = root.listFiles((directory, name) -> name.startsWith(ENTRY_PREFIX) &&
                (name.endsWith(".gray") || name.endsWith(".alpha") || name.endsWith(".rgba")));
        if (files == null) return;
    ...
    byte[] colorPlane(Entry entry) throws IOException {
        if (entry.rgba != null) return entry.rgba;
        byte[] rgba = readColorPlane(entry.id);
        return rgba == null ? SleepImageTransforms.expandGrayToRgba(entry.gray, entry.alpha) : rgba;
    }

    private byte[] readColorPlane(String id) throws IOException {
        File file = rgbaFile(id);
        if (!file.exists()) return null;
        int expected = Math.multiplyExact(bytes, 4);
        if (!file.isFile() || file.length() != expected) throw new IOException("invalid RGBA size");
        try (FileInputStream input = new FileInputStream(file)) {
            byte[] rgba = input.readNBytes(expected + 1);
            if (rgba.length != expected) throw new IOException("short RGBA plane");
            return rgba;
        }
    }

    private File rgbaFile(String id) { return new File(root, ENTRY_PREFIX + id + ".rgba"); }
    ...
    private int firstRemovable(List<Entry> entries) {
        for (int index = 0; index < entries.size(); ++index) {
            if (!entries.get(index).id.equals(builtInId)) return index;
        }
        return -1;
    }

    private int findBuiltIn(List<Entry> entries) {
        for (int index = 0; index < entries.size(); ++index) {
            if (entries.get(index).id.equals(builtInId)) return index;
        }
        return -1;
    ...
    private int findLegacyBuiltIn(List<Entry> entries) {
        for (int index = 0; index < entries.size(); ++index) {
            if (entries.get(index).builtIn && isLegacyBuiltInId(entries.get(index).id)) {
                return index;
            }
        }
        return -1;
    ...
    private boolean isLegacyBuiltInId(String id) {
        return (!builtInId.equals(BUILT_IN_ID) && id.equals(BUILT_IN_ID)) ||
                (builtInId.equals("built-in-fallback-aura-c-v2") &&
                        id.equals("built-in-fallback-aura-c-v1"));
    }
