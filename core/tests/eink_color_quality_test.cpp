...
int main() {
  ...
  // Equal code-space luma is not equal color: 38*75 == 75*38.
  history.Accept(std::array<std::uint8_t,4>{75,0,0,255});
  Require(history.Prepare(std::array<std::uint8_t,4>{0,38,0,255},intermediate)==1);
  // A different buffer size cannot inherit the previous target's history.
  Require(history.Prepare(target,intermediate)==0 && intermediate.empty());
  // Invalid acceptance invalidates history rather than retaining stale color.
  history.Accept(std::array<std::uint8_t,3>{0,0,0});
  Require(history.Prepare(std::array<std::uint8_t,4>{0,38,0,255},intermediate)==0);
  std::cout << "PASS\n";
}
