...
void PresentationAdapter::Start() {
  ...
  if (impl_->stopping) {
    // Stop is terminal for this single-owner worker. A silent no-op keeps an
    // accidental post-teardown restart fail-closed.
    return;
  }
  impl_->started = true;
...
bool PresentationAdapter::Enqueue(Frame frame) {
  {
    ...
    // Require at least one payload the selected backend can understand.
...
void PresentationAdapter::WorkerMain() {
  while (true) {
    ...
    // Backend ownership is intentionally serial.
