...
std::uint8_t Luma(const std::uint8_t* pixel, RgbaLayout layout) {
  ...
  // Fixed-point luma conversion:
  // (38 * R + 75 * G + 15 * B) >> 7.
  return static_cast<std::uint8_t>((38 * red + 75 * green + 15 * blue) >> 7);
}

std::uint8_t PanelGray(const std::uint8_t* pixel, RgbaLayout layout) {
  // Quantize to the high-nibble 16-level grayscale domain. The backend may
  // use a different conversion policy; this helper does not prescribe one.
